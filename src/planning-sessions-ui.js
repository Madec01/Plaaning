...
function psProjectTranches(project){try{return window.PM&&PM.tranches?PM.tranches(project.id):["common"].concat(project.tranche==="both"?["1","2"]:["1","2"].indexOf(project.tranche)>=0?[project.tranche]:[]);}catch(e){return["common"];}}
function psFillTranches(preferred,forceChoice){var form=document.getElementById("psSessionForm");if(!form)return"";var project=psProject(form.elements.projectId.value),scopes=project?psProjectTranches(project):[],value=scopes.indexOf(preferred)>=0?preferred:"";if(!value&&!forceChoice)value=project&&["1","2"].indexOf(project.tranche)>=0?project.tranche:"common";var placeholder=forceChoice&&!value?'<option value="">Choisir une tranche</option>':"";form.elements.tranche.innerHTML=placeholder+scopes.map(function(scope){return '<option value="'+psEscape(scope)+'"'+(scope===value?' selected':'')+'>'+psEscape(psTrancheLabel(scope))+'</option>';}).join("");form.elements.tranche.value=value;return value;}
